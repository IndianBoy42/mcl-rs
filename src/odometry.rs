...
impl<LOM: LinearOdometryModel<N>, N: Scalar + Copy> OdometryModel<N> for LOM {
    fn update_one(&self, _dt: N, _pose: Pose<N>) -> Pose<N> {
        // self.mean(dt, pose) + randomly_sample_from_gaussian
        todo!();
    }
    fn update_all(&self, _dt: N, _poses: &mut [Pose<N>]) {
...
where
    ...
    M: OdometryModel<N>,
{
    fn mean(&self, _dt: N, _curr: Pose<N>) -> Pose<N> {
        todo!()
    }

    fn covariance(&self, _dt: N, _curr: Pose<N>) -> PoseCovariance<N> {
        todo!()
    }
}
